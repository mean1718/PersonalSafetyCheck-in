...
const registerUser = async (req, res) => {
  try {
    ...
    if (name.length < 2) {
      return res
        .status(400)
        .json(validationError("Name must be at least 2 characters."));
    }
    if (!isValidEmail(email)) {
      return res
        .status(400)
        .json(validationError("Please enter a valid email address."));
    }
    if (password.length < 8) {
      return res
        .status(400)
        .json(validationError("Password must be at least 8 characters."));
    }
    if (!isValidPhone(phone)) {
      return res
        .status(400)
        .json(validationError("Please enter a valid phone number."));
    }
